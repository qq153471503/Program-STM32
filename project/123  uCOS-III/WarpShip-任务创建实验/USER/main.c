...
int main(void)
{
	...
	POINT_COLOR = RED;
	LCD_ShowString(50,30,72,16,16,"uC/OS-III");
	LCD_ShowString(50,50,224,16,16,"Tasks create and delete test");
...
void lcd1_task(void *p_arg)
{
	...
	POINT_COLOR = BLUE;
	LCD_ShowString(41,81,80,16,16," LCD1 Run:");
	OS_CRITICAL_EXIT();
	while(1)
	{
		num++;
		...
		LCD_Fill(41,101,145,199,color[num%14]);
		LCD_ShowxNum(128,81,num,1,16,0);
...
void lcd2_task(void *p_arg)
{
	...
	POINT_COLOR = BLUE;
	LCD_ShowString(151,81,80,16,16," LCD2 Run:");
	OS_CRITICAL_EXIT();
	while(1)
	{
		num++;
		printf("LCD2任务已经运行%d次\r\n",num);
		LCD_ShowxNum(232,81,num,2,16,0);
