...
void lcd_task(void *pdata)
{
	...
	GUI_DrawBitmap(&LogoTable,320/2/2,240/2/2);  //绘制ALIENTEK图标
	OSTimeDlyHMSM(0,0,4,0,OS_OPT_TIME_PERIODIC,&err);//延时4s
	while(1)
	{
		Xmag += 100;
		Ymag += 100;
		if(Xmag>=1500)Xmag = 100;
		if(Ymag>=1500)Ymag = 100;
		GUI_SetBkColor(GUI_BLUE);
		GUI_Clear();
		GUI_DispStringHCenterAt(" GUI_DrawBitmapEx TEST",320/2, 0);
		GUI_DrawBitmapEx(&LogoTable,320/2,240/2,160/2,120/2,Xmag,Ymag); //按照比例绘制位图
		GUI_Delay(500);
	}
}
