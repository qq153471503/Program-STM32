...
//********************************************************************************
//V1.2修改说明
//支持了SPFD5408的驱动,另外把液晶ID直接打印成HEX格式.方便查看TFT_LCD驱动IC.
//V1.3
//加入了快速IO的支持
...
{ 					
  ...
  FSMC_NORSRAMInit(&FSMC_NORSRAMInitStructure);  //初始化FSMC配置
	...
	}else if(lcddev.id==0x9320)//测试OK.
	{
		...
		LCD_WriteReg(0x02,0x0700);	//TFT_LCD Driver Waveform Contral.
	...
	}else if(lcddev.id==0x5408)
	{
		...
		LCD_WriteReg(0x02,0x0700);//TFT_LCD Driving Waveform Contral 
	...
	}else if(lcddev.id==0X1963)
	{
		...
		LCD_WR_REG(0xB0);		//设置TFT_LCD模式
		LCD_WR_DATA(0x20);		//24位模式
		LCD_WR_DATA(0x00);		//TFT 模式 
	
		LCD_WR_DATA((SSD_HOR_RESOLUTION-1)>>8);//设置TFT_LCD水平像素
		LCD_WR_DATA(SSD_HOR_RESOLUTION-1);		 
		LCD_WR_DATA((SSD_VER_RESOLUTION-1)>>8);//设置TFT_LCD垂直像素
		...
		LCD_WR_REG(0xBA);
		LCD_WR_DATA(0X01);	//GPIO[1:0]=01,控制TFT_LCD方向
		
		LCD_SSD_BackLightSet(100);//背光设置为最亮
	}		 
	LCD_Display_Dir(0);		//默认为竖屏0 
	LCD_LED=1;				//点亮背光
	LCD_Clear(WHITE);
}  
//清屏函数
